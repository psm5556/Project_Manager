...
import { createTechItem, updateTechItem } from '../../api'
import type { TechItem } from '../../types'

interface Props { projectId: number; item?: TechItem; onClose: () => void }

export function TechItemModal({ projectId, item, onClose }: Props) {
  const qc = useQueryClient()
  const [name, setName] = useState(item?.name ?? '')
  const [desc, setDesc] = useState(item?.description ?? '')

  const createMut = useMutation({
    mutationFn: () => createTechItem({ project_id: projectId, name, description: desc }),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['tech_items', projectId] }); toast.success('Tech Item 생성됨'); onClose() },
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    item ? updateMut.mutate() : createMut.mutate()
  }
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100 dark:border-slate-700">
          <div className="flex items-center gap-2.5">
            ...
            <h2 className="font-semibold text-[14px] text-slate-900 dark:text-slate-100">
              {item ? 'Tech Item 편집' : 'Tech Item 생성'}
            </h2>
          </div>
        ...
        <form onSubmit={handleSubmit} className="px-5 py-4 space-y-4">
          <div>
            ...
            <input
              ...
              placeholder="Tech Item 이름" autoFocus
            />
          </div>
          <div>
